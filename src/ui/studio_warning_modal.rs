...
impl StudioWarningModal {
    ...
    pub fn handle_event(&mut self, event: &UiEvent, sw: f32, sh: f32) -> StudioWarningResult {
        ...
        match event {
            ...
            UiEvent::MousePress { x, y } | UiEvent::DoubleClick { x, y } => {
                ...
                let confirm_btn = Rect { x: start_x, y: btn_y, width: btn_w, height: 30.0 };
                let cancel_btn = Rect { x: start_x + btn_w + gap, y: btn_y, width: btn_w, height: 30.0 };
    ...
    pub fn render<'a>(&'a self, quads: &mut Vec<QuadInstance>, labels: &mut Vec<LabelInfo<'a>>, sw: f32, sh: f32) {
        ...
        labels.push(LabelInfo {
            ...
            bounds: Rect { x: card.x, y: card.y + 14.0, width: card.width, height: 24.0 },
        ...
        labels.push(LabelInfo {
            ...
            bounds: Rect { x: card.x + 20.0, y: card.y + 46.0, width: card.width - 40.0, height: 16.0 },
            h_align: HAlign::Center, v_align: VAlign::Center,
            overflow: Overflow::Clip, padding: 0.0,
        ...
        labels.push(LabelInfo {
            ...
            bounds: Rect { x: card.x + 20.0, y: card.y + 66.0, width: card.width - 40.0, height: 16.0 },
            h_align: HAlign::Center, v_align: VAlign::Center,
            overflow: Overflow::Clip, padding: 0.0,
            font_size_override: Some(11.0), color_override: Some([200, 200, 210]), font_family_override: None,
        });
        ...
        labels.push(LabelInfo {
            ...
            bounds: Rect { x: card.x + 20.0, y: card.y + 86.0, width: card.width - 40.0, height: 16.0 },
            h_align: HAlign::Center, v_align: VAlign::Center,
            overflow: Overflow::Clip, padding: 0.0,
            font_size_override: Some(11.0), color_override: Some([200, 200, 210]), font_family_override: None,
        });
        ...
        let buttons = [
            (start_x, t("studio_warning.continue"), [0.20, 0.50, 0.30, 1.0]),
            (start_x + btn_w + gap, t("studio_warning.cancel"), [0.55, 0.25, 0.20, 1.0]),
        ];
        ...
        for (bx, label, color) in buttons {
            quads.push(QuadInstance {
                ...
                border_color: [0.5, 0.5, 0.55, 0.5], border_width: 1.0, border_radius: 4.0,
                shadow_offset: [0.0; 2], shadow_color: [0.0; 4], shadow_blur: 0.0,
                rotation: 0.0, _padding: [0.0; 2],
            });
            labels.push(LabelInfo {
                text: label,
                bounds: Rect { x: bx, y: btn_y, width: btn_w, height: 30.0 },
                h_align: HAlign::Center, v_align: VAlign::Center,
                overflow: Overflow::Clip, padding: 0.0,
                font_size_override: Some(11.0), color_override: Some([230, 230, 235]), font_family_override: None,
            });
        }
    }
}
