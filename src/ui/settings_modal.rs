...
impl SettingsModal {
    ...
    pub fn handle_event(&mut self, event: &UiEvent, screen_w: f32, screen_h: f32) -> SettingsModalResult {
        let card = card_rect(screen_w, screen_h);

        match event {
            UiEvent::KeyInput { text } if text == "\x1b" => {
                SettingsModalResult::Close
            }
            UiEvent::MouseMove { x, y } => {
                let list_x = card.x + 20.0;
                let list_y = card.y + 126.0;
                let list_w = card.width - 40.0;
                let list_rect = Rect { x: list_x, y: list_y, width: list_w, height: FONT_LIST_H };
            ...
            UiEvent::Scroll { x, y, delta, .. } => {
                ...
                let list_rect = Rect { x: list_x, y: list_y, width: list_w, height: FONT_LIST_H };
                if list_rect.contains(*x, *y) {
                    let max_scroll = (self.available_fonts.len() as f32 * FONT_ITEM_H - FONT_LIST_H).max(0.0);
                    self.font_scroll_offset = (self.font_scroll_offset - delta * 30.0).clamp(0.0, max_scroll);
                }
                SettingsModalResult::Consumed
            }
    ...
    pub fn render<'a>(&'a self, overlay_quads: &mut Vec<QuadInstance>, labels: &mut Vec<LabelInfo<'a>>, screen_w: f32, screen_h: f32) {
        ...
        for i in first_visible..self.available_fonts.len().min(first_visible + visible_count) {
