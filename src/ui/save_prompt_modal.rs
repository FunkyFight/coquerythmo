...
impl SavePromptModal {
    ...
    pub fn handle_event(&mut self, event: &UiEvent, sw: f32, sh: f32) -> SavePromptResult {
        ...
        match event {
            ...
            UiEvent::MousePress { x, y } | UiEvent::DoubleClick { x, y } => {
                ...
                let save_btn = Rect { x: start_x, y: btn_y, width: btn_w, height: 30.0 };
                let discard_btn = Rect { x: start_x + btn_w + gap, y: btn_y, width: btn_w, height: 30.0 };
                let cancel_btn = Rect { x: start_x + 2.0 * (btn_w + gap), y: btn_y, width: btn_w, height: 30.0 };
    ...
    pub fn render<'a>(&'a self, quads: &mut Vec<QuadInstance>, labels: &mut Vec<LabelInfo<'a>>, sw: f32, sh: f32) {
        ...
        labels.push(LabelInfo {
            ...
            bounds: Rect { x: card.x + 20.0, y: card.y + 46.0, width: card.width - 40.0, height: 20.0 },
            h_align: HAlign::Center, v_align: VAlign::Center,
            overflow: Overflow::Clip, padding: 0.0,
        ...
        let buttons = [
            ...
            (start_x + btn_w + gap, t("save_prompt.discard"), [0.55, 0.25, 0.20, 1.0]),
            (start_x + 2.0 * (btn_w + gap), t("save_prompt.cancel"), [0.25, 0.25, 0.30, 1.0]),
        ];
