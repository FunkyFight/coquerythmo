...
impl FileExplorerModal {
    pub fn new(request: FileExplorerRequest) -> Self {
        let current_dir = resolve_initial_dir(request.initial_dir.as_deref());
        let address = current_dir.to_string_lossy().into_owned();
        let filename = request.initial_filename.unwrap_or_default();
        let sidebar = build_sidebar(&request.extra_locations);
        let mut modal = Self {
            title: request.title,
            mode: request.mode,
            ...
            active_field: None,
            address_input: TextInputState::new(),
            name_filter_input: TextInputState::new(),
            filename_input: TextInputState::new(),
