...
fn y_to_slot(y: f32, zone: &Rect) -> f32 {
    ...
    let slot_index = (relative_y / total_slot_h)
        .floor()
        .clamp(0.0, constants::NUM_SLOTS - 1.0);
...
pub fn render_lines<'a>(
    ...
    for line in project.lines() {
        ...
        if state.syllable_mode
            && (is_hovered || state.syllable_drag.as_ref().map(|d| d.line_id) == Some(line.id))
        {
            ...
            if !breaks.is_empty() {
                ...
                let use_ratios = if ratios.len() == breaks.len() + 1 {
                    ratios
                } else {
                    &default_ratios
                };
                ...
                match drag.handle {
                    ...
                    DragHandle::Body => {
                        ...
                        let new_y_slot = if candidate != drag.original_y_slot {
                            ...
                            let orig_slot_idx =
                                (drag.original_y_slot * constants::NUM_SLOTS).round();
                            let orig_center = ctx.zone.y
                                + constants::RULER_HEIGHT
                                + orig_slot_idx * total_slot_h
                                + total_slot_h / 2.0;
...
pub fn render_studio_rythmo<'a>(
    ...
    for line in project.lines() {
        ...
        if !line.text.is_empty() && line.text != "\u{2191}" && line.text != "\u{2193}" {
            ...
            let breaks = crate::syllable::syllable_breaks(&line.text, &lang_cfg);
            let use_segments =
                !line.syllable_ratios.is_empty() && line.syllable_ratios.len() == breaks.len() + 1;
            if use_segments {
                let chars: Vec<char> = line.text.chars().collect();
                let mut seg_x = x1;
                let mut prev_break = 0usize;
