...
impl LintTooltipState {
    pub fn new(diagnostics: &[crate::lint::Diagnostic], cursor_x: f32, cursor_y: f32) -> Self {
        ...
        let entries = diagnostics
            .iter()
            .map(|diagnostic| LintTooltipEntry {
    ...
    fn rect(&self, screen_w: f32, font_size: f32) -> Rect {
        ...
        let longest = self
            ...
            .iter()
            .flat_map(|entry| {
                entry.lines.iter().enumerate().map(move |(index, line)| {
                    line.chars().count()
                        + if index == 0 {
                            if entry.severity == crate::lint::Severity::Error {
                                16
                            } else {
                                17
                            }
        ...
        Rect {
            x,
            y: self.cursor_y + TOOLTIP_OFFSET_Y,
            width,
            height,
        }
    }
    ...
    pub fn render_quads(&self, screen_w: f32) -> Vec<QuadInstance> {
        ...
        vec![QuadInstance {
            ...
            color: [0.18, 0.18, 0.20, 0.97],
            color_bottom: [0.14, 0.14, 0.16, 0.97],
    ...
    pub fn render_labels(&self, screen_w: f32) -> Vec<LabelInfo<'_>> {
        ...
        for (index, entry) in self.entries.iter().enumerate() {
            for (line_index, line) in entry.lines.iter().enumerate() {
                let y = rect.y
                    + TOOLTIP_PADDING_V
                    + entry_line as f32 * (font_size + LINT_TOOLTIP_LINE_GAP);
                if line_index == 0 {
                    ...
                    labels.push(LabelInfo {
                        ...
                        color_override: Some(if entry.severity == crate::lint::Severity::Error {
                            [255, 92, 92]
                        } else {
                            [255, 196, 54]
                        }),
                    ...
                    labels.push(LabelInfo {
                        text: line,
                        bounds: Rect {
                            x: rect.x + TOOLTIP_PADDING_H + prefix_width,
                            y,
                            width: rect.width - TOOLTIP_PADDING_H * 2.0 - prefix_width,
                            height: font_size,
                        },
                        h_align: HAlign::Left,
                        v_align: VAlign::Center,
                    ...
                    labels.push(LabelInfo {
                        ...
                        bounds: Rect {
                            x: rect.x + TOOLTIP_PADDING_H,
                            y,
                            width: rect.width - TOOLTIP_PADDING_H * 2.0,
                            height: font_size,
                        },
                        h_align: HAlign::Left,
                        v_align: VAlign::Center,
...
fn wrap_message(message: &str, font_size: f32, severity: crate::lint::Severity) -> Vec<String> {
    let max_chars =
        ((LINT_TOOLTIP_MAX_WIDTH - TOOLTIP_PADDING_H * 2.0) / (font_size * 0.56)).floor() as usize;
    let prefix_len = if severity == crate::lint::Severity::Error {
        16
    } else {
        17
    };
    ...
    while words.peek().is_some() {
        let capacity = if first {
            max_chars.saturating_sub(prefix_len).max(12)
        } else {
            max_chars
        };
        ...
        while let Some(word) = words.peek().copied() {
            ...
            if !line.is_empty() && needed > capacity {
                break;
            }
            if !line.is_empty() {
                line.push(' ');
