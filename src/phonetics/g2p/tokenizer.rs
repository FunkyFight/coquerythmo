...
pub fn tokenize(line: &NormalizedLine, language: Language) -> Vec<RawToken> {
    ...
    while i < chars.len() {
        ...
        if ch.is_ascii_digit() {
            ...
            while i < chars.len() {
                ...
                if c == ' '
                    ...
                    && chars.get(i.wrapping_sub(1)).is_some_and(|p| p.ch.is_ascii_digit())
                {
                    i += 1;
                    continue;
                }
        ...
        if is_word_char(ch) {
            ...
            while i < chars.len() && (is_word_char(chars[i].ch) || chars[i].ch == '\'') {
                if chars[i].ch == '\'' {
                    ...
                    if language == Language::French {
                        ...
                        let next_is_letter =
                            chars.get(i + 1).is_some_and(|c| c.ch.is_alphabetic());
            ...
            if i > start && tokens.last().is_none_or(|t| t.end_grapheme != chars[i - 1].original_grapheme + 1) {
                let text: String = chars[start..i].iter().map(|c| c.ch).collect();
                if !text.is_empty() {
...
mod tests {
    ...
    fn french_elision_splits_prefix_and_word() {
        ...
        assert!(result.iter().any(|(text, kind, _)| text == "d" && *kind == TokenKind::ElidedPrefix));
    ...
    fn punctuation_becomes_own_token() {
        ...
        assert!(result
            .iter()
            .any(|(text, kind, _)| text == "!" && *kind == TokenKind::Punctuation));
        assert_eq!(result.iter().filter(|(_, kind, _)| *kind == TokenKind::Word).count(), 2);
    }
    ...
    fn original_ranges_point_into_display_text() {
        ...
        let ca = result.iter().find(|(text, _, _)| text == "ça").expect("ça token");
        assert_eq!(ca.2, (6, 8));
    }
}
